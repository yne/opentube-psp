...
int fd,sm,mode;unsigned size,type,tmp;//atom parsing
...
void*getVSample(u32 s,u32*len){
	...
	while(mode==2)sceKernelDelayThread(1000*50);
	return video;
}
...
char*onPlay(){
	if(mode==1){mode=2;Alert("paused");return NULL;}
	if(mode==2){mode=1;Alert("playin");return NULL;}
	Alert("start playback\n");
	//for(int i=0;i<100;i++)printf("%i %08X\n",i,f.Vstsz[i]);
	int ath,vth;sm=0,mode=1;//sm=sceKernelCreateSema("test",0x0,0,4,NULL);
...
char*onStop(){
	...
	Free(video);Free(audio);video=audio=NULL;
	Free(Voff); Free(Aoff);Voff=Aoff=NULL;
//free atom list
	Free(c);c=NULL;
