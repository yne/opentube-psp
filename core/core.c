...
char* findCodec(char* name,unsigned id){//load a codec from name or id
	...
	char*mime=name?strrchr(name,'.'):id2str(id);
	if(!mime)return "badname";
...
char* play(char* file){
	...
	if(fd<0)return "file error";
	Read(fd,&mime,4);
	if((err=findCodec(NULL,mime)))
		if((err=findCodec(file,0)))
			return(err);
...
int unload(SceSize args,void*argp){
	if(ot->dmx&&ot->dmx->stop)ot->dmx->stop();//stop current playing
	if(ot->gui&&ot->gui->stop)ot->gui->stop();//stop interface
	ot->io->unload();
...
int stop(){return TH("arakiri",unload);}//to be unable to return
int start(SceSize args,void*argp){
	if(!(ot->me->pool=Memalign(0x400000,0x200000)))return -1;//first malloc
	sceIoChdir(CWD);
	outfd=CWD[0]=='h'?1:sceIoOpen("out",PSP_O_WRONLY|PSP_O_CREAT|PSP_O_TRUNC,0777);
...
int module_start(SceSize args,char*argp){
	...
	return TH("OpenTube.bootstrap",start);
}