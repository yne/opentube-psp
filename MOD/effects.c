...
static u16 wavetab[4][64] = {
	{ /*Sine : 
	for(var i=0;i<64;i++)
		document.write("(u16)"+Math.floor((Math.sin(Math.PI*(i/32)))*255)+",");
	*/
...
static int effect_ST00_Process(MODFILE *mod, int channel, int ecol) {
	if (!mod->channels[channel].sample)return 0;
	u8 note = mod->channels[channel].last_note;
	u8 dnote = ((note >> 4) * 12) + (note & 0x0f);
 	dnote += mod->channels[channel].sample->relative_note;
 	dnote = ((dnote / 12) << 4) | (dnote % 12);
	u8 octave = dnote >> 4;
	u8 semitone = dnote & 0x0f;
	u8*count=&mod->channels[channel].effects[ecol].arpeggio_count;
	if (*count){
		if(*count == 1)semitone += mod->channels[channel].effects[ecol].cur_operand >> 4;
		if(*count == 2)semitone += mod->channels[channel].effects[ecol].cur_operand & 0x0f;
		if(semitone>11) {
			semitone -= 12;
			octave++;
		}
	}
	MODFILE_SetNote(mod, channel,mod->channels[channel].instrument->note[(octave << 4) | semitone],mod->channels[channel].sample->middle_c,mod->channels[channel].sample->finetune);
	if (++*count>2)*count=0;
	return 0;
}
static int effect_ST00_Stop(MODFILE *mod, int channel, int ecol) {
	...
	u8 dnote = ((note >> 4) * 12) + (note & 0x0f);
 	dnote += mod->channels[channel].sample->relative_note;
 	dnote = ((dnote / 12) << 4) | (dnote % 12);
	MODFILE_SetNote(mod, channel,mod->channels[channel].instrument->note[dnote],mod->channels[channel].sample->middle_c,mod->channels[channel].sample->finetune);
	return 0;
}
...
static int effect_ST30_Start(MODFILE *mod, int channel, int ecol, MOD_Note *note) {
	if (note->operand[ecol]) mod->channels[channel].effects[ecol].toneporta_bak = note->operand[ecol];
	else mod->channels[channel].effects[ecol].cur_operand = mod->channels[channel].effects[ecol].toneporta_bak;
	return EFFECT_DONOTCHANGEPERIOD | EFFECT_DONOTRESETSAMPLEPOS;
}
...
static int effect_ST40_Start(MODFILE *mod, int channel, int ecol, MOD_Note *note) {
	if (note->operand[ecol])
		mod->channels[channel].effects[ecol].vibrato_bak = note->operand[ecol];
	u8 operand = mod->channels[channel].effects[ecol].vibrato_bak;
	mod->channels[channel].effects[ecol].cur_operand = operand;
	if (operand & 0x0f)mod->channels[channel].effects[ecol].vibrato_depth = operand & 0x0f;
	if (operand & 0xf0)mod->channels[channel].effects[ecol].vibrato_freq = (operand >> 4) & 0x0f;
	return 0;
}
...
static void doSTVolumeSlide(MODFILE *mod, int channel, int ecol) {
	...
	else if (mod->channels[channel].effects[ecol].cur_operand & 0x0f) /* Down */
...
static int effect_STa0_Process(MODFILE *mod, int channel, int ecol) {
	if (mod->speedcounter)doSTVolumeSlide(mod, channel, ecol);	
	return 0;
}
...
static int effect_STc0_Start(MODFILE *mod, int channel, int ecol, MOD_Note *note) {
	if (note->operand[ecol] > 64) mod->channels[channel].voiceInfo.volume = 64;
	else mod->channels[channel].voiceInfo.volume = note->operand[ecol];
...
static int effect_S3Mq0_Process(MODFILE *mod, int channel, int ecol) {
	...
	if (!mod->speedcounter)return 0;
	if (mod->channels[channel].effects[ecol].retrig_count < (mod->channels[channel].effects[ecol].cur_operand & 0x0f))return 0;
	if (mod->channels[channel].effects[ecol].retrig_count >= (mod->channels[channel].effects[ecol].cur_operand & 0x0f))
		mod->channels[channel].effects[ecol].retrig_count = 0;
	/* Retrigger */
	...
	mod->channels[channel].voiceInfo.forward = TRUE;
	/* Volume slide */
	if (!(mod->channels[channel].effects[ecol].cur_operand & 0xf0))return 0;
	switch ((mod->channels[channel].effects[ecol].cur_operand >> 4) & 0x0f) {
		...
		case 5:
			...
			break;
		case 6:
			mod->channels[channel].voiceInfo.volume = (mod->channels[channel].voiceInfo.volume * 2) / 3;
			break;
		case 7:
		...
		case 13:
			...
			break;
		case 14: {
			u8 tmpvol = (mod->channels[channel].voiceInfo.volume * 3) / 2;
			mod->channels[channel].voiceInfo.volume = tmpvol>64?64:tmpvol;
			break;}
		case 15:
...
static int effect_S3Mr0_Start(MODFILE *mod, int channel, int ecol, MOD_Note *note) {
	if (note->operand[ecol])mod->channels[channel].effects[ecol].tremolo_bak = note->operand[ecol];
	else mod->channels[channel].effects[ecol].cur_operand=mod->channels[channel].effects[ecol].tremolo_bak;
	u8 operand = mod->channels[channel].effects[ecol].cur_operand;
	mod->channels[channel].effects[ecol].tremolo_base = mod->channels[channel].voiceInfo.volume;
	if (operand & 0x0f)mod->channels[channel].effects[ecol].tremolo_depth = operand & 0x0f;
	if (operand & 0xf0)mod->channels[channel].effects[ecol].tremolo_freq = (operand >> 4) & 0x0f;
	return 0;
}
static int effect_S3Mr0_Process(MODFILE *mod, int channel, int ecol) {
	...
	delta *= mod->channels[channel].effects[ecol].tremolo_depth;
	delta >>= 7;
	u16 v = mod->channels[channel].effects[ecol].tremolo_base + delta;
	if (v > 64)v = 64;
	if (v & 0xff80)v = 0;
	mod->channels[channel].voiceInfo.volume = v;
 	mod->channels[channel].effects[ecol].tremolo_sintabpos += mod->channels[channel].effects[ecol].tremolo_freq;
...
static int effect_S3Mt0_Start(MODFILE *mod, int channel, int ecol, MOD_Note *note) {
	if (note->operand[ecol] > 0)MODFILE_SetBPM(mod, note->operand[ecol]);
	return 0;
}
